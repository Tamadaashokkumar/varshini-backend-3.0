  ...
  try {
    ...
    for (const product of products) {
      ...
      const totalSoldLast30Days =
        salesData.length > 0 ? salesData[0].totalSold : 0;
    ...
    if (res) {
      res
        ...
        .json({
          success: true,
          message: `Inventory analytics updated for ${updatesCount} products`,
        });
    }
  } catch (error) {
...
export const getInventoryForecast = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch forecast" });
  }
};
